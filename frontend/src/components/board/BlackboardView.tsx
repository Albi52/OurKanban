...
import { Plus, Trash2, Pencil, Image as ImageIcon, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, X } from 'lucide-react'
    ...
    <div className="flex flex-1 flex-col overflow-auto" data-testid="blackboard-view">
      ...
      <div className="flex flex-1 items-start justify-center gap-2 pb-8">
        <div className="flex flex-col items-center gap-2">
          <EdgeButton icon={ArrowUp} label="Add row" onClick={() => handleGrow('row', 'TOP')} />

          <div className="flex items-center gap-2">
            <EdgeButton icon={ArrowLeft} label="Add column" onClick={() => handleGrow('col', 'LEFT')} vertical />
              ...
              {board.elements.map((el) => (
                <div
                  ...
                  className={`group relative overflow-hidden rounded-xl border border-border bg-background shadow-sm ${
                    canManage(el) ? 'cursor-grab active:cursor-grabbing' : ''
                  }`}
                ...
                >
                  ...
                  {editingElementId === el.id ? (
                    ...
                    el.textContent && (
                      <p
                        className={`relative z-10 line-clamp-[8] break-words p-2 text-xs text-foreground-secondary ${
                          el.imageUrl ? 'mt-auto bg-background/80 backdrop-blur-sm' : ''
                        }`}
                      >
                        {el.textContent}
                      </p>
            ...
            <EdgeButton icon={ArrowRight} label="Add column" onClick={() => handleGrow('col', 'RIGHT')} vertical />
          </div>

          <EdgeButton icon={ArrowDown} label="Add row" onClick={() => handleGrow('row', 'BOTTOM')} />
        </div>
      </div>
...
function EdgeButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={label}
      className={`flex shrink-0 items-center justify-center rounded-full border border-dashed border-border text-muted-foreground-subtle hover:border-border-hover hover:text-foreground-secondary ${
        vertical ? 'h-9 w-9' : 'h-9 w-9'
      }`}
    >
